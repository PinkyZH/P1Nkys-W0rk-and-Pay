...
from PySide6.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QTableWidget, QTableWidgetItem, QPushButton, QComboBox, QLabel
...
class AuditLogDialog(QDialog):
    def __init__(self, session_factory: sessionmaker, lang: str="de", parent=None):
        ...
        self.lang = lang
        self.setWindowTitle(t("tools.audit_log", self.lang) if t("tools.audit_log", self.lang)!="tools.audit_log" else "Audit-Log")
    ...
    def _build(self):
        ...
        self.combo_user = QComboBox(); self.combo_user.addItem(t("admin_users.name", self.lang) + ": " + t("calendar.all", self.lang) if t("calendar.all", self.lang)!="calendar.all" else "Alle", None)
        ...
        filter_row.addWidget(QLabel(t("admin_users.username", self.lang))); filter_row.addWidget(self.combo_user); filter_row.addWidget(btn_reload)
        layout.addLayout(filter_row)

        self.table = QTableWidget(0, 7)
        self.table.setHorizontalHeaderLabels([t("calendar.date", self.lang), "User", "Aktion", "Entity", "Entity-ID", "Before", "After"])
    ...
    def _load(self):
        ...
        for a in rows:
            ...
            it_time = QTableWidgetItem(fmt_datetime(a.created_at, self.lang)); it_time.setData(Qt.EditRole, str(a.created_at))
