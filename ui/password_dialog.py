...
class PasswordDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.edit_confirm = QLineEdit(); self.edit_confirm.setEchoMode(QLineEdit.EchoMode.Password)
        ...
        btns.addWidget(self.btn_change); btns.addWidget(self.btn_cancel)
    ...
    def _on_generate(self):
        ...
        self.edit_new.setText(pw); self.edit_confirm.setText(pw)

    def _on_change(self):
        new = self.edit_new.text().strip()
        conf = self.edit_confirm.text().strip()
        if new != conf:
            QMessageBox.warning(self, tr("pwd.title", self.lang), tr("pwd.mismatch", self.lang)); return
        if len(new) < 6:
            QMessageBox.warning(self, tr("pwd.title", self.lang), "Min. 6 Zeichen erforderlich." if self.lang == "de" else "Min. 6 characters required."); return
        self.accept()
