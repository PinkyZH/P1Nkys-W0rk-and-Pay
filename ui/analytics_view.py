...
from PySide6.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QComboBox, QDateEdit, QPushButton
...
class AnalyticsDialog(QDialog):
    ...
    def _build(self):
        ...
        row = QHBoxLayout()
        self.d_from = QDateEdit(); self.d_from.setCalendarPopup(True); self.d_from.setDate(QDate.currentDate().addMonths(-1))
        self.d_to   = QDateEdit(); self.d_to.setCalendarPopup(True); self.d_to.setDate(QDate.currentDate())
        btn = QPushButton("Aktualisieren")
        for w in (self.d_from, self.d_to, btn): row.addWidget(w)
        layout.addLayout(row)

        # Charts
        self.chart_hours = QChart(); self.chart_hours.setTitle("Stunden pro Tag")
        self.view_hours = QChartView(self.chart_hours); layout.addWidget(self.view_hours)

        self.chart_types = QChart(); self.chart_types.setTitle("Typ-Verteilung (Stunden)")
        self.view_types = QChartView(self.chart_types); layout.addWidget(self.view_types)

        self.chart_gross = QChart(); self.chart_gross.setTitle("Brutto (Total) pro Monat")
        self.view_gross = QChartView(self.chart_gross); layout.addWidget(self.view_gross)
    ...
    def _reload(self):
        ...
        self.chart_hours.removeAllSeries(); self.chart_hours.addSeries(s_line)
        axX = QValueAxis(); axX.setTitleText("Tage"); axY = QValueAxis(); axY.setTitleText("Stunden")
        self.chart_hours.createDefaultAxes(); self.chart_hours.setAxisX(axX, s_line); self.chart_hours.setAxisY(axY, s_line)

        # Typ-Pie
        pie = QPieSeries()
        for k,v in types_hours.items():
            pie.append(k, v)
        self.chart_types.removeAllSeries(); self.chart_types.addSeries(pie)
        ...
        bar.append(sset)
        self.chart_gross.removeAllSeries(); self.chart_gross.addSeries(bar)
        axC = QBarCategoryAxis(); axC.append(cats)
        axV = QValueAxis(); axV.setTitleText("CHF")
        self.chart_gross.createDefaultAxes(); self.chart_gross.setAxisX(axC, bar); self.chart_gross.setAxisY(axV, bar)
