...
class EntryDialog(QDialog):
    ...
    def _build(self):
        ...
        self.ed_end = QTimeEdit()
        self.ed_break = QSpinBox();
        self.ed_break.setRange(0, 600);
        self.ed_break.setSuffix(" min")

        # Stunden + Typ
        self.ed_hours = QDoubleSpinBox();
        self.ed_hours.setRange(0.0, 24.0);
        self.ed_hours.setDecimals(2);
        self.ed_hours.setSingleStep(0.25)
        ...
        self.ed_wage = QDoubleSpinBox();
        self.ed_wage.setRange(0.0, 1000.0);
        self.ed_wage.setDecimals(2)
        self.ed_overtime = QDoubleSpinBox();
        self.ed_overtime.setRange(0.0, 24.0);
        self.ed_overtime.setDecimals(2)
        self.ed_location = QLineEdit()

        # Bezahlung-Override
        self.cmb_pay = QComboBox()
        for label, val in [("100%", 1.0), ("90%", 0.9), ("80%", 0.8), ("70%", 0.7), ("60%", 0.6), ("50%", 0.5),
                           ("Unbezahlt", 0.0), ("Benutzerdefiniert…", -2.0)]:
            self.cmb_pay.addItem(label, val)
        self.sp_pay_custom = QDoubleSpinBox()
        self.sp_pay_custom.setRange(0.0, 2.0);
        self.sp_pay_custom.setDecimals(2);
        self.sp_pay_custom.setValue(1.0)
