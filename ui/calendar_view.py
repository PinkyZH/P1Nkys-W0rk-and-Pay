...
from PySide6.QtGui import QPainter, QPen, QColor, QShortcut, QKeySequence, QIcon
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QComboBox, QPushButton, QDateEdit, QTableWidget,
    QTableWidgetItem, QCalendarWidget, QSplitter, QLabel, QMenu, QLineEdit,
    QAbstractItemView, QHeaderView, QGroupBox, QCheckBox, QFileDialog, QMessageBox
)
from sqlalchemy.orm import sessionmaker

from config import DEFAULT_WAGE_PRESETS as _W
from core.models import UserProfile, User
from core.services.settings_service import get_user_pref, set_user_pref
...
_TYPES_FALLBACK = {
    "de": {"work": "Arbeit", "vacation": "Urlaub", "sick": "Krank", "holiday": "Feiertag", "off": "Frei",
           "other": "Sonstiges", "appointment": "Arzttermin", "hospital": "Spital"},
    "en": {"work": "Work", "vacation": "Vacation", "sick": "Sick", "holiday": "Holiday", "off": "Off",
           "other": "Other", "appointment": "Appointment", "hospital": "Hospital"},
    "sr": {"work": "Rad", "vacation": "Odmor", "sick": "Bolovanje", "holiday": "Praznik", "off": "Slobodno",
           "other": "Ostalo", "appointment": "Pregled", "hospital": "Bolnica"},
}


def _t(lang: str, key: str) -> str:
    val = tr(f"calendar.{key}", lang)
    if val != f"calendar.{key}":
        return val
    # Fallbacks
    return {
        ...
        "type_filter": "Typ-Filter", "all": "Alle", "search": "Suche Notiz/Ort", "user": "Nutzer",
...
class MarkingCalendar(QCalendarWidget):
    def __init__(self, parent=None):
        ...
        self._color_map_multi: Dict[QDate, List[QColor]] = {}
    ...
    def paintCell(self, painter: QPainter, rect, d: QDate):
        ...
        if d in self._marks:
            colors = self._color_map_multi.get(d, [])
            if not colors:
                return
            margin, diameter, gap = 3, 5, 2
            x_right = rect.right() - margin
            y_top = rect.top() + margin + 2
...
class CalendarMainWidget(QWidget):
    def __init__(self, session_factory: sessionmaker, user_id: int, lang: str = "de", is_admin: bool = False,
        ...
        self._marks_cache: Dict[Tuple[int, int, int], tuple[set[QDate], Dict[QDate, List[QColor]]]] = {}
        self._build()
    ...
    def _build(self):
        ...
        if self.is_admin:
            ...
            top.addWidget(QLabel(_t(self.lang, "user")));
        ...
        self.combo_view.addItems([_t(self.lang, "view_day"), _t(self.lang, "view_week"),
                                  _t(self.lang, "view_month"), _t(self.lang, "view_year")])
        ...
        for code in ["WORK", "VACATION", "SICK", "ACCIDENT", "HOLIDAY", "OFF", "OTHER"]:
            self.combo_type_filter.addItem(_type_label(self.lang, code), code)

        self.search_edit = QLineEdit();
        self.search_edit.setPlaceholderText(_t(self.lang, "search"))
        ...
        # Analytics Sichtbarkeit (persistiert pro User)
        ...
        for w in [self.combo_view, self.date_picker, self.combo_type_filter, self.search_edit,
                  ...
                  btn_import_csv, btn_export_csv, btn_pdf, btn_xlsx, self.chk_anx]:
            top.addWidget(w)
        ...
        self.table = QTableWidget(0, 10)  # +2: Tagessold Brutto/Netto
        headers = [
            _t(self.lang, "date"), _t(self.lang, "start"), _t(self.lang, "end"), _t(self.lang, "break"),
        ...
        # Doppelklick Edit & Kontextmenü
        self.table.itemDoubleClicked.connect(lambda *_: self._edit_entry())
        ...
        row_range.addWidget(btn_apply_range)
        ...
        layout.addWidget(self.analytics_box, stretch=4)
        ...
        # Icons & Farben
        btn_new.setIcon(icon("list-add"))
        btn_edit.setIcon(icon("document-edit"))
        btn_delete.setIcon(icon("edit-delete"))
        btn_refresh.setIcon(icon("view-refresh"))
        btn_pdf.setIcon(icon("application-pdf"))
        btn_xlsx.setIcon(icon("x-office-spreadsheet"))
        btn_export_csv.setIcon(icon("text-csv"))
        btn_import_csv.setIcon(icon("document-import"))

        btn_new.setStyleSheet("QPushButton{background:#b7f7bf;} QPushButton:hover{background:#a7f0b1;}")
        btn_edit.setStyleSheet("QPushButton{background:#e0e0e0;} QPushButton:hover{background:#d5d5d5;}")
        btn_delete.setStyleSheet("QPushButton{background:#ffb3b3;} QPushButton:hover{background:#ffa0a0;}")
        btn_refresh.setStyleSheet("QPushButton{background:#bfe3ff;} QPushButton:hover{background:#a9d8ff;}")
        btn_pdf.setStyleSheet("QPushButton{background:#ff6b6b; color:white;} QPushButton:hover{background:#ff5a5a;}")
        btn_xlsx.setStyleSheet("QPushButton{background:#2e7d32; color:white;} QPushButton:hover{background:#256628;}")
        btn_export_csv.setStyleSheet("QPushButton{background:#ffa726;} QPushButton:hover{background:#ff9800;}")
        btn_import_csv.setStyleSheet("QPushButton{background:#fff176;} QPushButton:hover{background:#ffee58;}")
    ...
    # ---------- Resize -> Notiz/Ort automatisch breiter ----------
    ...
    # ---------- Kontextmenü ----------
    def _open_ctx_menu(self, pos: QPoint):
        idx = self.table.indexAt(pos)
        if idx.isValid():
            self.table.selectRow(idx.row())

        m = QMenu(self)
        a_new = m.addAction(icon("list-add"), _t(self.lang, "new_entry"))
        a_edit = m.addAction(icon("document-edit"), _t(self.lang, "edit_entry"))
        a_del = m.addAction(icon("edit-delete"), _t(self.lang, "delete_entry"))
        m.addSeparator()
        a_ref = m.addAction(icon("view-refresh"), _t(self.lang, "refresh"))
        m.addSeparator()
        a_pdf = m.addAction(icon("application-pdf"), "PDF Export")
        a_xlsx = m.addAction(icon("x-office-spreadsheet"), "Excel Export")
        a_csvx = m.addAction(icon("text-csv"), "CSV Export")
        a_csvi = m.addAction(icon("document-import"), "CSV Import")
        ...
        if act == a_new:
            ...
            self._import_csv_entries()

    # ---------- User-Combo ----------
    def _load_users_into_combo(self):
        cb = getattr(self, "user_combo", None)
        if cb is None:
            return
        ...
        try:
            ...
            with self.session_factory() as s:
                ...
                profiles = {p.user_id: p for p in s.execute(select(UserProfile)).scalars().all()}
            for u in users:
                p = profiles.get(u.id)
                if p and (p.first_name or p.last_name):
                    label = f"{(p.first_name or '').strip()} {(p.last_name or '').strip()} ({u.username})".strip()
                else:
                    label = u.username or f"ID {u.id}"
                cb.addItem(label, u.id)
        finally:
    ...
    def _view_mode(self) -> str:
        txt = (self.combo_view.currentText() or "").strip().lower()
        if "tag" in txt or "day" in txt:
            return "DAY"
        if "woche" in txt or "week" in txt:
            return "WEEK"
        if "jahr" in txt or "year" in txt:
            return "YEAR"
        return "MONTH"
    ...
    def _go_prev(self):
        ...
        if vm == "MONTH":
            self.calendar.setSelectedDate(qd.addMonths(-1));
            return
        if vm == "YEAR":
            self.calendar.setSelectedDate(qd.addYears(-1));
            return
        self.calendar.setSelectedDate(qd.addDays(-(7 if vm == "WEEK" else 1)))
    ...
    def _period_wages(self, hours_sum: float) -> Tuple[float, float]:
        ...
        vac = _v("vac_pct")
        hol = _v("holiday_pct")
        m13 = _v("thirteenth_pct")
        exp = _v("expenses_per_hour")
        ahv = _v("ahv_pct")
        nbu = _v("nbu_pct")
        ktg = _v("ktg_pct")
        bvg = _v("bvg_pct", 0.0)
    ...
    def _reload(self, initial: bool = False):
        ...
        # SICK/ACCIDENT -> 80% von 8h als Geldleistung (nicht Stunden!)
        ...
        for e in show_list:
            ...
            it_date.setData(Qt.ItemDataRole.UserRole, e.id)
            self.table.setItem(r, 0, it_date)
            self.table.setItem(r, 1, QTableWidgetItem(e.start_time.strftime("%H:%M") if e.start_time else ""))
            self.table.setItem(r, 2, QTableWidgetItem(e.end_time.strftime("%H:%M") if e.end_time else ""))

            pause_val = int(e.break_minutes or 0)
            self.table.setItem(r, 3, NumericItem(pause_val, str(pause_val)))

            hours_val = float(e.hours or 0.0)
            self.table.setItem(r, 4, NumericItem(hours_val, f"{hours_val:.2f}"))
            ...
            self.table.setItem(r, 5, typ_item)

            self.table.setItem(r, 6, QTableWidgetItem(e.note or ""))
            self.table.setItem(r, 7, QTableWidgetItem(e.location or ""))

            # Tagessold (Brutto/Netto)
            gross_per_hour, net_per_hour = self._period_wages(1.0)
            if typ_code == "WORK":
            ...
            it_g.setForeground(QColor("#d93025"))
            it_n = QTableWidgetItem(f"{day_net:.2f}");
            it_n.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter);
            it_n.setForeground(QColor("#137333"))
            self.table.setItem(r, 8, it_g);
            self.table.setItem(r, 9, it_n)
        ...
        # Analytics neu zeichnen
        self._rebuild_analytics()

    # ---------- Punkte/Marker im Monatskalender ----------
    def _refresh_month_marks(self):
        ...
        if key not in self._marks_cache:
            ...
            for e in entries:
                types_by_day.setdefault(e.date, set()).add((e.entry_type or "WORK").upper())
    ...
    def _edit_entry(self):
        ...
        with self.session_factory() as s:
            e = get_entry(s, eid)
        if not e:
            QMessageBox.warning(self, _t(self.lang, "title"), "Entry not found.")
            self._reload();
            return
    ...
    # ---------- Export/Import ----------
    def _current_view_entries(self):
        start, end = self._range_for_view()
        with self.session_factory() as s:
            entries = list_entries_by_range(s, self.view_user_id, start, end)
        vm = self._view_mode();
        selected_d = self._selected_date()
        if vm == "DAY":
            entries = [e for e in entries if e.date == selected_d]
        q = (self.search_edit.text() or "").strip().lower()
        type_code = self.combo_type_filter.currentData()
        out = []
        for e in entries:
    ...
    def _export_pdf(self):
        ...
        # Dateiname mit Monat/Jahr
        from datetime import date as _date
        import re
        try:
            y, m = entries[0].date.year, entries[0].date.month
        except Exception:
            today = _date.today();
        ...
        def _san(s: str) -> str:
            ...
            s = re.sub(r"\s+", "_", s)
            import re as _re
            return _re.sub(r"[^A-Za-z0-9_.\-äöüÄÖÜß]", "", s) or "NA"
        ...
        try:
            with self.session_factory() as s:
                p = s.execute(select(UserProfile).where(UserProfile.user_id == self.view_user_id)).scalar_one_or_none()
                if p:
                    ln, fn, em = p.last_name or ln, p.first_name or fn, p.employer or em
                else:
    ...
    def _export_csv(self):
        ...
        def _calendar_export_filename(last: str, first: str, employer: str, ext: str) -> str:
            from datetime import date as _date
            today = _date.today().strftime("%Y-%m-%d")
            return f"Cal-Export_{_safe_name(last)}_{_safe_name(first)}_{_safe_name(employer)}_{today}.{ext}"

        path, _ = QFileDialog.getSaveFileName(self, "CSV speichern",
                ...
                with self.session_factory() as s:
                    for row in reader:
                        try:
                            ...
                            brk_i = int(get(row, "break_minutes") or 0)
                            hrs_f = float((get(row, "hours") or "0").replace(",", "."))
    ...
    def _export_analytics_pdf(self):
        ...
        if not entries:
            ...
            return
        # Standardname
        path, _ = QFileDialog.getSaveFileName(self, "PDF speichern", "analytics.pdf", "PDF (*.pdf)")
        if not path: return

        def _worked(t):
            return (t or "").upper() in ("WORK", "ARBEIT")

        total_hours = sum(float(e.hours or 0) for e in entries if _worked(getattr(e, "entry_type", "")))
        gross = net = 0.0
        try:
            g, n = self._period_wages(total_hours);
            gross, net = float(g or 0), float(n or 0)
        except Exception:
            pass

        styles = getSampleStyleSheet()
        title = Paragraph("Analytics – Übersicht", styles["Title"])
        info = Paragraph(
            f"Arbeitsstunden gesamt: <b>{total_hours:.2f}</b> &nbsp; | &nbsp; "
            f"Bruttolohn: <b>{gross:.2f}</b> &nbsp; | &nbsp; "
            f"Nettolohn: <b>{net:.2f}</b>", styles["Normal"]
        )

        days: Dict[date, float] = {}
        for e in entries:
            d = getattr(e, "date", None)
            if d and _worked(getattr(e, "entry_type", "")):
                days[d] = days.get(d, 0.0) + float(e.hours or 0)

        data = [["Datum", "Arbeitsstunden"]] + [[d.isoformat(), f"{h:.2f}"] for d, h in sorted(days.items())]
        table = Table(data, repeatRows=1)
        ...
        doc = SimpleDocTemplate(path, pagesize=landscape(A4), title="Analytics")
        elems = [title, Spacer(1, 8), info, Spacer(1, 12), table];
        doc.build(elems)
        QMessageBox.information(self, "PDF", f"Exportiert: {path}")
    ...
    def _export_analytics_xlsx(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Analytics Excel speichern", "analytics.xlsx", "Excel (*.xlsx)")
        if not path: return

        wb = openpyxl.Workbook();
        ws = wb.active;
        ws.title = "Analytics"
        tmp1 = NamedTemporaryFile(suffix=".png", delete=False)
        tmp2 = NamedTemporaryFile(suffix=".png", delete=False)
        self.fig_hours.savefig(tmp1.name, dpi=150, bbox_inches="tight")
        self.fig_gross.savefig(tmp2.name, dpi=150, bbox_inches="tight")
    ...
    def _export_analytics_png(self):
        target = getattr(self, "analytics_box", None)
        if target is None or not hasattr(target, "grab"):
            target = getattr(self, "table", None)
        if target is None or not hasattr(target, "grab"):
            QMessageBox.warning(self, "PNG", "Keine geeignete Ansicht zum Exportieren gefunden.");
            return
        path, _ = QFileDialog.getSaveFileName(self, "PNG speichern", "analytics.png", "PNG (*.png)")
        if not path: return
        try:
            pixmap = target.grab()
            if not pixmap.save(path, "PNG"):
                QMessageBox.warning(self, "PNG", "Konnte PNG nicht speichern.");
                return
            QMessageBox.information(self, "PNG", f"Exportiert: {path}")
        except Exception as ex:
            QMessageBox.critical(self, "PNG", f"Fehler beim Export: {ex}")
