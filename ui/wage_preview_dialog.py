...
from PySide6.QtWidgets import (
    ...
    QLineEdit, QSpinBox, QDoubleSpinBox, QPushButton, QGroupBox, QGridLayout
)
...
class WagePreviewDialog(QDialog):
    ...
    def _build(self):
        ...
        # --- Szenario-Header ---
        hdr = QLabel(tr("wprev.header", self.lang) if tr("wprev.header", self.lang) != "wprev.header"
                     else "Vergleich links: IST  •  rechts: WAS-WÄRE-WENN")
        ...
        self.ro_hourly  = _RO()
        self.ro_vac     = _RO(); self.ro_hol = _RO(); self.ro_13 = _RO()
        self.ro_exp     = _RO()
        self.ro_ahv     = _RO(); self.ro_nbu = _RO(); self.ro_ktg = _RO(); self.ro_bvg = _RO()
        self.ro_week    = _RO()
        for label, w in [
            (tr("profile.hourly_wage", self.lang), self.ro_hourly),
            (tr("profile.vac_pct_label", self.lang) if tr("profile.vac_pct_label", self.lang) != "profile.vac_pct_label" else "Feriengeld %", self.ro_vac),
            (tr("profile.holiday_pct_label", self.lang) if tr("profile.holiday_pct_label", self.lang) != "profile.holiday_pct_label" else "Feiertage %", self.ro_hol),
            (tr("profile.thirteenth_pct_label", self.lang) if tr("profile.thirteenth_pct_label", self.lang) != "profile.thirteenth_pct_label" else "13. Monatslohn %", self.ro_13),
            (tr("profile.expenses_per_hour_label", self.lang) if tr("profile.expenses_per_hour_label", self.lang) != "profile.expenses_per_hour_label" else "Spesen/Std.", self.ro_exp),
            ("AHV/IV/EO/ALV %", self.ro_ahv), ("NBU/BU %", self.ro_nbu), ("KTG %", self.ro_ktg), ("BVG %", self.ro_bvg),
            (tr("profile.weekly_hours_label", self.lang) if tr("profile.weekly_hours_label", self.lang) != "profile.weekly_hours_label" else "Wochenstunden", self.ro_week),
        ]:
            fL.addRow(label, w)

        # Rechte Spalte (WHAT-IF, editierbar + Delta-Stunden)
        grp_if = QGroupBox(tr("wprev.whatif", self.lang) if tr("wprev.whatif", self.lang) != "wprev.whatif" else "Was-wäre-wenn")
        fR = QFormLayout(grp_if)
        self.if_hourly = QDoubleSpinBox(); self.if_hourly.setRange(0, 1000); self.if_hourly.setDecimals(2)
        self.if_vac    = QDoubleSpinBox(); self.if_vac.setRange(0, 100);  self.if_vac.setDecimals(2)
        self.if_hol    = QDoubleSpinBox(); self.if_hol.setRange(0, 100);  self.if_hol.setDecimals(2)
        self.if_13     = QDoubleSpinBox(); self.if_13.setRange(0, 100);   self.if_13.setDecimals(2)
        self.if_exp    = QDoubleSpinBox(); self.if_exp.setRange(0, 1000); self.if_exp.setDecimals(2)
        self.if_ahv    = QDoubleSpinBox(); self.if_ahv.setRange(0, 100);  self.if_ahv.setDecimals(2)
        self.if_nbu    = QDoubleSpinBox(); self.if_nbu.setRange(0, 100);  self.if_nbu.setDecimals(2)
        self.if_ktg    = QDoubleSpinBox(); self.if_ktg.setRange(0, 100);  self.if_ktg.setDecimals(2)
        self.if_bvg    = QDoubleSpinBox(); self.if_bvg.setRange(0, 100);  self.if_bvg.setDecimals(2)
        self.if_week   = QDoubleSpinBox(); self.if_week.setRange(0, 100); self.if_week.setDecimals(2)

        self.if_extra_hours = QDoubleSpinBox(); self.if_extra_hours.setRange(-300, 300); self.if_extra_hours.setDecimals(2)
        self.if_extra_hours.setToolTip(tr("wprev.extra_hours_tip", self.lang) if tr("wprev.extra_hours_tip", self.lang) != "wprev.extra_hours_tip" else "Zusatzstunden (±) für Monatsvergleich")

        for label, w in [
            (tr("profile.hourly_wage", self.lang), self.if_hourly),
            (tr("profile.vac_pct_label", self.lang) if tr("profile.vac_pct_label", self.lang) != "profile.vac_pct_label" else "Feriengeld %", self.if_vac),
            (tr("profile.holiday_pct_label", self.lang) if tr("profile.holiday_pct_label", self.lang) != "profile.holiday_pct_label" else "Feiertage %", self.if_hol),
            (tr("profile.thirteenth_pct_label", self.lang) if tr("profile.thirteenth_pct_label", self.lang) != "profile.thirteenth_pct_label" else "13. Monatslohn %", self.if_13),
            (tr("profile.expenses_per_hour_label", self.lang) if tr("profile.expenses_per_hour_label", self.lang) != "profile.expenses_per_hour_label" else "Spesen/Std.", self.if_exp),
            ("AHV/IV/EO/ALV %", self.if_ahv), ("NBU/BU %", self.if_nbu), ("KTG %", self.if_ktg), ("BVG %", self.if_bvg),
            (tr("profile.weekly_hours_label", self.lang) if tr("profile.weekly_hours_label", self.lang) != "profile.weekly_hours_label" else "Wochenstunden", self.if_week),
            (tr("wprev.extra_hours", self.lang) if tr("wprev.extra_hours", self.lang) != "wprev.extra_hours" else "Zusatzstunden (±) / Monat", self.if_extra_hours),
        ]:
            fR.addRow(label, w)
        ...
        grp_quick = QGroupBox(tr("wprev.quick.title", self.lang) if tr("wprev.quick.title", self.lang) != "wprev.quick.title"
                              else "Schnell-Vergleich (nur Lohn & Stunden)")
        ql = QGridLayout(grp_quick)
        ql.addWidget(QLabel(""), 0, 0)
        ql.addWidget(QLabel(tr("wprev.col_ist", self.lang) if tr("wprev.col_ist", self.lang) != "wprev.col_ist" else "IST"), 0, 1, alignment=Qt.AlignCenter)
        ql.addWidget(QLabel(tr("wprev.col_if", self.lang) if tr("wprev.col_if", self.lang) != "wprev.col_if" else "Was-wäre-wenn"), 0, 2, alignment=Qt.AlignCenter)

        # Zeile 1: Stundenlohn
        ql.addWidget(QLabel(tr("profile.hourly_wage", self.lang)), 1, 0)
        self.q_ist_wage = QDoubleSpinBox(); self.q_ist_wage.setRange(0, 1000); self.q_ist_wage.setDecimals(2)
        self.q_if_wage  = QDoubleSpinBox(); self.q_if_wage.setRange(0, 1000);  self.q_if_wage.setDecimals(2)
        ql.addWidget(self.q_ist_wage, 1, 1); ql.addWidget(self.q_if_wage, 1, 2)

        # Zeile 2: Stunden
        ql.addWidget(QLabel(tr("wprev.hours", self.lang) if tr("wprev.hours", self.lang) != "wprev.hours" else "Stunden"), 2, 0)
        self.q_ist_hours = QDoubleSpinBox(); self.q_ist_hours.setRange(0, 10000); self.q_ist_hours.setDecimals(2)
        self.q_if_hours  = QDoubleSpinBox(); self.q_if_hours.setRange(0, 10000);  self.q_if_hours.setDecimals(2)
        ql.addWidget(self.q_ist_hours, 2, 1); ql.addWidget(self.q_if_hours, 2, 2)

        # --- Ergebnis-Block (IST vs IF) ---
        ...
        def _mk_label(color_hex: str):
            lab = QLabel("-"); lab.setAlignment(Qt.AlignRight)
            lab.setStyleSheet(f"font-weight:600; color:{color_hex};")
            return lab

        ql.addWidget(QLabel(tr("payroll.gross", self.lang) if tr("payroll.gross", self.lang) != "payroll.gross" else "Bruttolohn"), 3, 0)
        self.q_ist_g = _mk_label("#d93025")  # rot
        self.q_if_g  = _mk_label("#d93025")
        ql.addWidget(self.q_ist_g, 3, 1); ql.addWidget(self.q_if_g, 3, 2)

        ql.addWidget(QLabel(tr("payroll.net", self.lang) if tr("payroll.net", self.lang) != "payroll.net" else "Nettolohn"), 4, 0)
        self.q_ist_n = _mk_label("#137333")  # grün
        self.q_if_n  = _mk_label("#137333")
        ql.addWidget(self.q_ist_n, 4, 1); ql.addWidget(self.q_if_n, 4, 2)

        root.addWidget(grp_quick)

        # Buttons – Neuberechnen
        row_btn = QHBoxLayout()
        btn_apply = QPushButton(tr("dialogs.common.refresh", self.lang) if tr("dialogs.common.refresh", self.lang) != "dialogs.common.refresh" else "Aktualisieren")
        row_btn.addStretch(1); row_btn.addWidget(btn_apply)
    ...
    def _load_effective_params(self):
        ...
        self.q_ist_hours.setValue(174.0)
        self.q_if_hours.setValue(174.0)

    # ------------- Rechnen -------------
    def _effective_rates_from(self, params: Dict[str, float]) -> Tuple[float, float]:
        """berechnet (brutto/std, netto/std) aus den Parametern"""
    ...
    def _recalc_all(self):
        ...
        # Prognosen (IST-Raten)
        self._recalc_forecasts(gph_ist, nph_ist)

        # Schnell-Vergleich updaten
        self._recalc_quick()

    def _recalc_quick(self):
        """Berechnet den kompakten IST/IF-Vergleich nur aus Lohn & Stunden (inkl. Abgaben/Spesen-Logik aus Profil)."""
        # IST
    ...
    def _recalc_forecasts(self, gph: float, nph: float):
        ...
        end_m = (start_m.replace(month=start_m.month+1, day=1) - timedelta(days=1)) if start_m.month < 12 \
            else (start_m.replace(year=start_m.year+1, month=1, day=1) - timedelta(days=1))
        # nächster Monat
        nm_start = (end_m + timedelta(days=1))
        nm_end = (nm_start.replace(month=nm_start.month+1, day=1) - timedelta(days=1)) if nm_start.month < 12 \
            else (nm_start.replace(year=nm_start.year+1, month=1, day=1) - timedelta(days=1))
        ...
        def period_total(d1: date, d2: date) -> Tuple[float, float]:
            ...
            sick_days  = sum(1 for e in entries if (e.entry_type or "").upper() == "SICK")
