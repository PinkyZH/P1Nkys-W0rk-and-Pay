...
def calc_surcharges_for_entry(d: date, start_t: time | None, end_t: time | None, break_minutes: int | None) -> dict:
    ...
    dt0 = datetime.combine(d, start_t);
    dt1 = datetime.combine(d, end_t)
...
def surcharge_amounts_from_minutes(minutes_map: dict, gross_per_hour: float, net_per_hour: float) -> Tuple[
    float, float]:
