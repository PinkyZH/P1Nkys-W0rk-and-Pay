...
def list_entries_by_range(session: Session, user_id: int, start_d: date, end_d: date) -> List[WorkEntry]:
    stmt = select(WorkEntry).where(and_(WorkEntry.user_id == user_id, WorkEntry.date >= start_d, WorkEntry.date <= end_d)).order_by(WorkEntry.date.asc(), WorkEntry.start_time.asc().nullsfirst())
    return list(session.execute(stmt).scalars().all())

def list_entries_by_date(session: Session, user_id: int, d: date) -> List[WorkEntry]:
    stmt = select(WorkEntry).where(and_(WorkEntry.user_id == user_id, WorkEntry.date == d)).order_by(WorkEntry.start_time.asc().nullsfirst())
    return list(session.execute(stmt).scalars().all())

def create_entry(session: Session, user_id: int, **kwargs) -> WorkEntry:
    e = WorkEntry(user_id=user_id)
    for k in ("date","start_time","end_time","break_minutes","hours","entry_type","note","hourly_override","overtime_hours","location"):
...
def delete_entry(session: Session, entry_id: int) -> bool:
    ...
    session.delete(e); session.commit(); return True

def totals_for_range(session: Session, user_id: int, start_d: date, end_d: date) -> Tuple[float, int]:
    stmt = select(func.coalesce(func.sum(WorkEntry.hours), 0.0), func.count(WorkEntry.id)).where(and_(WorkEntry.user_id == user_id, WorkEntry.date >= start_d, WorkEntry.date <= end_d))
...
def dates_with_entries_by_month(session: Session, user_id: int, year: int, month: int) -> dict:
    ...
    stmt = select(WorkEntry.date, func.count(WorkEntry.id)).where(and_(WorkEntry.user_id == user_id, WorkEntry.date >= first, WorkEntry.date <= last)).group_by(WorkEntry.date)
    res = {}
