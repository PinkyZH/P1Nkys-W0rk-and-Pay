...
from core.db_migrate import run_light_migrations
from core.migrate_encrypt import migrate_encrypt
...
handler = logging.handlers.RotatingFileHandler(os.path.join(LOG_DIR, "app.log"), maxBytes=1_000_000, backupCount=5,
                                               encoding="utf-8")
...
def main() -> int:
    ...
    with Session() as s:
        try:
            changed = migrate_encrypt(s)
            # optional: print/log
        except Exception as ex:
            # Nicht hart failen – aber loggen
            print(f"[encrypt-migrate] Fehler: {ex}")
